...
Object.entries(variants).forEach(([key, value]) => {
  ...
  const theme = (accent: string, name: string, file: string) => {
    const theme: JBTheme = {
      name: name,
      dark: !isLatte,
      ...
      icons: {
        ColorPalette: {
          ...
          "Checkbox.Foreground.Selected": colors[accent],
  ...
  theme("mauve", `Catppuccin ${capitalize(key)}`, `${key}.theme.json`)
  accents.forEach((accent) => theme(accent, `Catppuccin ${capitalize(key)} ${capitalize(accent)}`, `${key}.${accent}.theme.json`));
});
